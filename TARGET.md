...
1. **Keep the privileged battery green.** The self-hosted workflow is now the
   ...
   causes. Current routing: there is no durable repo runner for this path; the
   privileged workflow only runs by explicit broker dispatch to a disposable L1
   runner with `self-hosted`, `m80-privileged-e2e`, `kvm`, and a unique
   `m80-e2e-*` label. Main CI run `26510307333` is green on
   `7ab4f3f42d04f4947fccdbfd9ddabbff2d06a9f4`, proving the workflow policy
   lint and non-privileged gates after the broker-only cutover.
