...
| `Subtree::Drop` | Writes `1` to leaf `cgroup.kill` when that kernel file is present, waits briefly for `cgroup.procs` to drain, then removes the leaf cgroup directory with bounded `rmdir` retries; if any best-effort teardown step fails, logs through `tracing` and never panics. |
