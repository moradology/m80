//! Configuration loading: defaults → /etc/m80/config.toml → ~/.config/m80/config.toml
//! → M80_* env vars → caller-supplied flag overrides. Returns an [`EffectiveConfig`]
//! with each field tagged by its winning source.
...
/// Fields not recognized are silently ignored (they may be for other subsystems).
pub fn load(args_overrides: HashMap<String, String>) -> Result<EffectiveConfig, FcError> {
    let mut fields = defaults();

    // Layer 2: /etc/m80/config.toml
    let system_path = std::path::Path::new("/etc/m80/config.toml");
    ...
    // Layer 3: ~/.config/m80/config.toml
    if let Some(home) = home_dir() {
        let user_path = home.join(".config/m80/config.toml");
        if user_path.exists() {
    ...
    // Layer 4: M80_* environment variables.
    apply_env_layer(&mut fields);

    // Layer 5: caller-supplied flag overrides.
