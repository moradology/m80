...
#[path = "../common/mod.rs"]
mod common;
#[path = "installer_layout/config_preservation.rs"]
mod config_preservation;
...
mod remote_fetch;
#[path = "installer_layout/state_lock.rs"]
mod state_lock;
...
#[test]
fn install_bundle_layout_keeps_flat_wrapper_when_systemd_launch_is_available() {
    let bundle = write_release_bundle(None);
    let host = HostPrereqFixture::new();
    let install_temp = tempfile::tempdir().unwrap();
    let install_root = install_temp.path().join("install-root");
    ...
    assert_hardlinked(
        &version_dir.join("bin/m80-jailer-harden"),
    ...
    assert!(
        ...
        "installed default profile must name the flat wrapper: {profile}"
    );
    assert!(
        profile.contains(
            &install_root
...
fn run_install_with_extra_args(
    bundle: &fixture::ReleaseBundleFixture,
    install_root: &Path,
    host: Option<&HostPrereqFixture>,
    extra_args: &[&str],
    ...
    run_install_url_with_extra_args(
        &format!("file://{}", bundle.tarball.display()),
        install_root,
        host,
        extra_args,
...
fn run_install_url(
    ...
    run_install_url_with_extra_args(bundle_url, install_root, host, &[], envs, env_removals)
}

fn run_install_url_with_extra_args(
    bundle_url: &str,
    install_root: &Path,
    host: Option<&HostPrereqFixture>,
    extra_args: &[&str],
