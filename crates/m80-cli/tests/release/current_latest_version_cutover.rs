...
fn current_latest_version_cutover_names_real_repair_tag() {
    ...
    assert!(
        cargo_toml.contains("version = \"0.2.14\""),
        "workspace package version must match the current repair tag"
    );
    assert!(
        runbook.contains("workspace package version `0.2.14`")
            && runbook.contains("expected release tag is `v0.2.14`"),
        "release runbook must document the real current repair version"
    );
    assert!(
        behavior.contains("matching stable tag is `v0.2.14`")
