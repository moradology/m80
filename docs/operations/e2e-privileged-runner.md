...
artifact directory and emits `install-provenance.json` before preflight. In
default `full` mode it installs `m80-guestd` from the checked-out commit and
rebuilds the guest image if the rootfs manifest points at a different daemon
hash. For `launch-only`, stage the matching `m80-guestd` artifact yourself.
