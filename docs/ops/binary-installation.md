...
sudo install -o root -g root -m 0644 firecracker-seccomp-filter.json /opt/firecracker/bin/firecracker-seccomp-filter.json
```
...
`m80-preflight` checks the configured Firecracker, jailer, and hardening-wrapper
paths against this manifest. It opens every manifest path with `O_NOFOLLOW`,
