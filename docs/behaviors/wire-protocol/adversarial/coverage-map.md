...
| Unsolicited response before request | `m80-g0v8.12.7` | `crates/m80-guestd-malicious` mode `unsolicited_response` writes a valid `exec_exit` frame for `unsolicited-response` immediately after accept, without reading the host request; `crates/m80-firecracker/tests/malicious/unsolicited_response.rs` runs it through the real-KVM path. | Synthetic coverage is partial: host lifecycle assumes one connection carries one active request, so unsolicited output is modeled once a caller is reading. | Current policy is reject-and-teardown for the affected channel via `WireProtocolError::RequestIdMismatch`. | Diagnostics name the request-id mismatch and fabricated id, no cross-request data leak occurs, and sandbox cleanup completes. | Closed for unsolicited guest response observed on an active request channel. |
| Unsolicited flood | `m80-g0v8.12.8` | `crates/m80-guestd-malicious` mode `unsolicited_flood` writes a bounded burst of fabricated `exec_exit` responses without reading the request; `crates/m80-firecracker/tests/malicious/dos_attacks.rs` runs it through the real-KVM path. | Bounded-buffer stream tests cover normal producer queues; L12.8 covers the adversarial unsolicited flood path. | Current policy is fail-fast on the first fabricated id via `WireProtocolError::RequestIdMismatch`; host does not read the flood into unbounded memory. | `/proc/self/status` snapshots bound host RSS growth, diagnostics name the fabricated id, and sandbox cleanup completes. | Closed for bounded unsolicited guest response flood. |
| Slowloris partial frame | `m80-g0v8.12.8` | `crates/m80-guestd-malicious` mode `slowloris` opens the channel, writes a prefix and short partial body, then stalls; `crates/m80-firecracker/tests/malicious/dos_attacks.rs` runs it through the real-KVM path. | `framing_parse_failure` covers closed short body; `m80-vsock::Channel::recv_raw_with_deadline` and `m80-firecracker` exec host-deadline units cover deadline expiry. | `WireProtocolError::ReadTimeout` within the caller's host exec deadline. | Reader thread joins within the test deadline, diagnostics record no-progress timeout, and sandbox cleanup completes. | Closed for guest-to-host no-progress partial-frame timeout when the caller sets a host deadline. |
...
- Truncated or disconnected frames surface as an under-read. Through lifecycle
  request handling, the user-visible error is `DisconnectBeforeTerminal` with
  a `DisconnectCause` distinguishing dead Firecracker, initial channel
  failure, mid-stream EOF, and host-requested clean close.
