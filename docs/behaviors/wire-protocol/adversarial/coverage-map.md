...
|---|---|---|---|---|---|---|
| Oversized frame length | `m80-g0v8.12.2` | Planned `oversized_length` malicious guestd emits a prefix larger than `MAX_FRAME_BYTES` without a body. | `crates/m80-proto/tests/framing_size_cap.rs::rejects_frame_above_4mib_with_oversized_error`; `crates/m80-guestd/tests/handle_connection.rs::oversized_initial_frame_drops_connection_without_response`; `crates/m80-firecracker/tests/wire_frame_boundaries_real_kvm.rs::frame_length_over_max_drops_current_channel_only` covers the host-to-guest/control direction. | `WireProtocolError::OversizedFrame` via `ProtoError::OversizedPayload`, before allocating the announced body. | Current channel closes, diagnostic names oversized payload size and limit, fresh channel remains usable when the sandbox policy allows it, sandbox cleanup completes. | Real malicious guest-to-host path remains open until `m80-g0v8.12.2` closes. |
| Truncated frame body | `m80-g0v8.12.3` | Planned `truncated_frame` malicious guestd writes a valid prefix, writes fewer body bytes than promised, then closes or stalls. | `crates/m80-proto/tests/framing_parse_failure.rs::short_frame_body_returns_unexpected_eof`; lifecycle protocol mapping covers EOF as `DisconnectBeforeTerminal`. | Typed under-read / disconnect error, currently `WireProtocolError::DisconnectBeforeTerminal` when surfaced through lifecycle receive. | Reader unblocks, request permit or activity guard is released, no stuck worker thread, cleanup can tear down the sandbox. | Real malicious guest-to-host under-read remains open until `m80-g0v8.12.3` closes. |
...
| Unknown envelope variant | `m80-g0v8.12.4` | Planned `unknown_variant` malicious guestd bypasses safe encoding and emits a valid envelope with an out-of-range payload kind. | Lower-level conversion tests reject unknown generated wire kinds through the protobuf conversion path; current evidence is synthetic only. | Typed wire-schema error naming the offending kind or tag. | Channel closes without panic, panic hook or thread join confirms no hidden panic, diagnostics include observed unknown tag. | Real malicious guest-to-host path remains open until `m80-g0v8.12.4` closes. |
| Response type mismatch | `m80-g0v8.12.5` | Planned `response_type_mismatch` malicious guestd reads a request and replies with the wrong valid response kind for the same `request_id`. | `crates/m80-firecracker/src/lifecycle/exec.rs` unit tests exercise wrong kind handling through `decode_frame_for_request`; file upload ack tests cover wrong upload/sequence shape. | Typed protocol error equivalent to response type mismatch, naming expected and observed payload kinds. | Affected channel closes, caller receives a clean sandbox-level error, subsequent sandbox use follows the documented channel-survival policy. | Real malicious guest-to-host path remains open until `m80-g0v8.12.5` closes. |
| Wrong or stale `request_id` | `m80-g0v8.12.6` | Planned `bogus_request_id` malicious guestd replies with a never-issued or stale request id over the actual vsock path. | `crates/m80-firecracker/src/lifecycle/exec.rs::tests::response_frame_rejects_stale_request_id`, `response_frame_rejects_missing_request_id`, and `cancel_ack_rejects_stale_request_id`; `crates/m80-guestd/tests/handle_connection.rs::cancel_wrong_request_id_returns_already_exited` covers guest-side cancel semantics. | `WireProtocolError::RequestIdMismatch` with expected and observed request ids. | Original request completes or times out cleanly per test mode, diagnostics include expected/observed ids, no cross-request data leak. | Real malicious guest-to-host path remains open until `m80-g0v8.12.6` closes. |
