...
| Slowloris partial frame | `m80-g0v8.12.8` | `crates/m80-guestd-malicious` mode `slowloris` opens the channel, writes a prefix and short partial body, then stalls; `crates/m80-firecracker/tests/malicious/dos_attacks.rs` runs it through the real-KVM path. | `framing_parse_failure` covers closed short body; `m80-firecracker` protocol unit tests map read timeout errors to `WireProtocolError::ReadTimeout`. | `WireProtocolError::ReadTimeout` within the bridge read-timeout bound. | Reader thread joins within the test deadline, diagnostics record no-progress timeout, and sandbox cleanup completes. | Closed for guest-to-host no-progress partial-frame timeout. |
