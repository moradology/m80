...
- `slowloris`: a malicious guest writes a valid in-cap frame prefix plus a
  ...
  host's vsock bridge read timeout bounds the wait and surfaces:
...
- `crates/m80-firecracker/src/lifecycle/protocol.rs` maps read timeouts after
  channel establishment to `WireProtocolError::ReadTimeout` instead of a raw
  transport error.
