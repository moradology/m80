...
- same-tag asset index row and checksum sidecar;
- bundle bytes and bundle checksum sidecar;
- metadata sidecar and metadata checksum sidecar;
- `install.sh` and `install.sh.sha256`;
- `m80-bootstrap-selector.tsv` and checksum sidecar;
- `m80-release-build.json` and checksum sidecar;
- public `SHA256SUMS`;
