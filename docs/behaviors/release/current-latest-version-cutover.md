...
The current repair source therefore uses workspace package version `0.2.8`.
The matching stable tag is `v0.2.8`; `v0.2.7` is intentionally rejected as an
...
so an unreleased local build now reports `0.2.8-dev` and remains barred from
using mutable `releases/latest` implicitly.

Fixture and release-script coverage uses `v0.2.8` as the packageable release
