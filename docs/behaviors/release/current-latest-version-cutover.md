...
The current repair source therefore uses workspace package version `0.2.11`.
The matching stable tag is `v0.2.11`; tags at or before the existing public
latest `v0.2.10` are intentionally rejected as old-release backfill candidates
...
so an unreleased local build now reports `0.2.11-dev` and remains barred from
using mutable `releases/latest` implicitly.

Fixture and release-script coverage uses `v0.2.11` as the packageable release
