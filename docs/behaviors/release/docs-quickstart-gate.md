...
- `common`: the latest install command or `sudo m80 run -- echo hello` from the
...
`sudo m80 run -- echo hello` command blocks fail the release URL contract tests. This
...
pinned `install.sh`, `m80 install`, and `sudo m80 run -- echo hello`; `m80 quickstart`
...
install snippet, the `sudo m80 run -- echo hello` smoke, the concise process-wrapper
...
`sudo m80 run -- echo hello` is the public proof command. Manual binary or
...
proofs must record `sudo m80 run -- echo hello` as both the display command and argv.
