...
returns `FcError::GuestdReadyTimeout`. A cgroup v2 subtree creation failure
after jailer launch returns `FcError::Cgroup`. All three paths force-kill the
pre-running Firecracker/jailer process, drop the jail/cgroup/storage locals,
remove the partial run directory, and release the admission permit.
