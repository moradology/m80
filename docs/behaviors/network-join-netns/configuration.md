...
`NetworkPolicy::JoinNetns { netns_path }` launches Firecracker inside a
caller-provided network namespace. m80 does not create the namespace, add links,
configure addresses, set routes, or install firewall rules for this mode. The
caller owns that setup and teardown.
...
before execing Firecracker.

This mode is separate from `NoEgress` and `AllowOutbound`. `NoEgress` still
creates no NIC. `AllowOutbound` still resolves to OutboundNat and is rejected by
the current `m80-firecracker` launch path until the outbound realizer is wired
in. `JoinNetns` is the explicit escape hatch for callers that already have a
namespace with the desired network policy.
