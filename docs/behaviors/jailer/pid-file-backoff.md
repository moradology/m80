...
waits for `<jail_root>/<firecracker-bin-basename>.pid`. That file is the
handoff point for the Firecracker process PID when the jailer forks,
daemonizes, or enters a new PID namespace. For the normal binary name this is
`firecracker.pid`; tests and configured wrapper binaries use their actual
executable basename.
...
- `crates/m80-jailer/src/materialized.rs::tests::jailer_pid_file_uses_exec_basename`
