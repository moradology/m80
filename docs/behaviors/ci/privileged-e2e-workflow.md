...
Beads: `m80-16hx7.7`, `m80-s3r28.2`

The privileged E2E workflow is the self-hosted CI lane for real-KVM behavior.
It targets runners labeled `self-hosted` and `kvm`, keeps GitHub permissions
read-only, and refuses fork pull-request execution by requiring same-repository
PRs plus the `run-e2e` label.
