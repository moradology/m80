...
instead of the durable `kvm` label. Manual dispatch exposes `runner_label`, and
the job resolves `M80_E2E_RUNNER_LABEL` from the dispatch input, repository
variable, or `kvm` default. The job records the selected label in substrate
diagnostics before running smoke or ignored tests.
...
caller-provided unique label plus `kvm`, starts the runner process, and waits
until GitHub reports the runner online. The operator then dispatches
`.github/workflows/e2e-privileged.yml` with that same label. GitHub assigns at
most one job to that runner, and the operator destroys the L1 VM after logs and
