...
then mapped into the range `3..=u32::MAX` using `3 + (raw % (u32::MAX - 2))`.
The same `vm_id` always produces the same CID across restarts; no allocation
table is needed.
...
`3..=u32::MAX`, keeping it strictly below the reserved vsock CIDs:
`VMADDR_CID_ANY = u32::MAX`, `VMADDR_CID_HOST = 2`, `VMADDR_CID_HYPERVISOR = 1`,
and the "any" address 0. The formula `3 + (raw % (u32::MAX - 2))` guarantees
the maximum output is `3 + (u32::MAX - 3) = u32::MAX`, and the minimum is `3`.
