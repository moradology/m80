...
| `M80_FIRECRACKER_SECCOMP_FILTER` | Host path to the Firecracker advanced seccomp filter bitcode. | `/opt/firecracker/bin/firecracker-seccomp-filter.bin` |
...
| `M80_NET_HELPER_BIN` | Host path to the m80 outbound network helper. | `/opt/m80/bin/m80-net-helper` |
