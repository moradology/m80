...
- reflink-cloned per-VM writable rootfs state;
...
- Phase A: reflink per-VM rootfs clone path with ext4 fallback behavior.
...
Phase C/F must also resolve compressed-erofs behavior. The PoC mounted a
compressed Rust toolchain image with `dax=always`, but mount success does not
prove file-level DAX page sharing for representative reads.
...
identity, pmem image digest set, and post-init state digest.

Phase D post-restore uniqueness currently uses a host-driven restore signal
plus explicit guest `RNDRESEEDCRNG`. The current stripped kernel disables ACPI,
so Linux `CONFIG_VMGENID` cannot consume Firecracker VMGenID. A future
