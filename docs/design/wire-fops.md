...
| `FileWriteRequest { path, bytes, mode }` | `FileWriteResponse { bytes_written, error }` | Creates/truncates one file. Parent directory must exist. Inline writes are bounded by the envelope cap. |
...
| `TooLarge` | Inline write exceeded the direct-write cap. |
