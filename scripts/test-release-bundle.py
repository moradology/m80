...
class ReleaseBundleTest(unittest.TestCase):
    def test_packages_release_bundle_with_metadata(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            inputs = fixture_inputs(root, release_tag="v0.2.12")
            out_dir = root / "out"
            ...
            self.assertEqual(metadata["release_tag"], "v0.2.12")
            self.assertEqual(metadata["m80_version"], "v0.2.12")
            self.assertEqual(metadata["package_version"], "0.2.12")
            ...
            self.assertEqual(metadata["guestd_package_version"], "0.2.12")
            ...
            self.assertIn("M80_RELEASE_TAG='v0.2.12'", public_install)
            ...
            self.assertEqual(index["release_tag"], "v0.2.12")
            self.assertEqual(len(index["assets"]), 1)
            asset = index["assets"][0]
            self.assertEqual(asset["name"], BUNDLE_NAME)
            self.assertEqual(
                asset["url"],
                f"https://github.com/moradology/m80/releases/download/v0.2.12/{BUNDLE_NAME}",
            )
            ...
            self.assertEqual(asset["m80_version"], "v0.2.12")
            ...
            self.assertEqual(selector[1], "release_tag\tv0.2.12")
            self.assertEqual(
                selector[2],
            ...
            self.assertEqual(build_manifest["m80_package_version"], "0.2.12")
            ...
            self.assertEqual(integrity["release_tag"], "v0.2.12")
            ...
            self.assertEqual(integrity["m80_package_version"], "0.2.12")
    ...
    def test_package_rejects_hardcoded_public_bundle_install_script(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            write_executable(
                ...
                "#!/bin/sh\n"
                "M80_RELEASE_TAG='@M80_RELEASE_TAG@'\n"
                "curl -fsSL https://github.com/moradology/m80/releases/download/v0.2.12/m80-linux-x86_64.tar.gz -o bundle.tgz\n"
                "bin/m80 install --bundle-url file://bundle.tgz\n",
            )
    ...
    def test_rendered_install_script_non_root_with_sudo_prints_pinned_retry_before_curl(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn(
                "curl -fsSL https://github.com/moradology/m80/releases/download/v0.2.12/install.sh | sudo sh",
                result.stderr,
            )
    ...
    def test_rendered_install_script_selects_verified_selector_before_bundle(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            base = "https://github.com/moradology/m80/releases/download/v0.2.12"
            self.assertEqual(
                urls,
                [
            ...
            self.assertIn("verified release tag=v0.2.12", result.stderr)
            self.assertIn(
            ...
            self.assertIn(
                "verified handoff binary=v0.2.12 "
        ...
        for exit_code, failure in cases:
            with self.subTest(exit_code=exit_code, failure=failure):
                with tempfile.TemporaryDirectory() as tmp:
                    ...
                    self.assertNotEqual(result.returncode, 0)
                    self.assertIn("release_tag=v0.2.12", result.stderr)
                    self.assertIn(f"asset={BOOTSTRAP_SELECTOR_NAME}", result.stderr)
                    self.assertIn(failure, result.stderr)
                    self.assertIn("verification_started=no", result.stderr)
                    self.assertIn(
                        f"url=https://github.com/moradology/m80/releases/download/v0.2.12/{BOOTSTRAP_SELECTOR_NAME}",
                        result.stderr,
                    )
    ...
    def test_rendered_install_script_rejects_unsigned_dev_fixture_before_bundle(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn(
                "retry pinned command: curl -fsSL --connect-timeout 10 --max-time 120 --retry 2 --retry-delay 1 https://github.com/moradology/m80/releases/download/v0.2.12/install.sh | sudo sh",
                result.stderr,
            )
    ...
    def test_release_upload_manifest_schema_derives_public_and_non_public_assets(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(manifest["release_tag"], "v0.2.12")
            self.assertEqual(
    ...
    def test_release_publish_receipt_writes_and_validates_publish_inputs(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(receipt["release_tag"], "v0.2.12")
            ...
            self.assertEqual(receipt["github_ref"], "refs/tags/v0.2.12")
    ...
    def test_release_evidence_bundle_writes_schema_entrypoint(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(bundle["release_tag"], "v0.2.12")
            self.assertEqual(bundle["commit_sha"], INTEGRITY_COMMIT_SHA)
            self.assertEqual(bundle["workflow_run_id"], "12345")
            self.assertEqual(bundle["m80_version"], "v0.2.12")
            self.assertEqual(bundle["resolved_install_tag"], "v0.2.12")
            ...
            for row in receipt_refs.values():
                ...
                self.assertEqual(row["release_tag"], "v0.2.12")
                ...
                [
                    {
                        ...
                        "bundle_m80_version": "v0.2.12",
                        "bundle_release_tag": "v0.2.12",
                        "guest_protocol_version": 1,
                        "lane_id": "hostless-quickstart",
                        "m80_release_tag": "v0.2.12",
                        "m80_version": "v0.2.12",
                        "manifest_schema_version": 5,
                        "requested_tag": "v0.2.12",
                        "resolved_install_tag": "v0.2.12",
                        "substrate": "hostless",
                    }
                ],
            )
    ...
    def test_release_evidence_bundle_rejects_receipt_tag_mismatch(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("expected=v0.2.12", result.stderr)
    ...
    def test_release_evidence_bundle_accepts_latest_resolved_identity(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(bundle["release_identity"][0]["resolved_install_tag"], "v0.2.12")
    ...
    def test_release_evidence_bundle_rejects_wrong_resolved_install_tag(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertNotEqual(result.returncode, 0)
            self.assertIn("resolved install tag mismatch: expected v0.2.12, got v9.9.9", result.stderr)
    ...
    def test_release_evidence_bundle_rejects_m80_version_mismatch(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertNotEqual(result.returncode, 0)
            self.assertIn("m80 version mismatch: expected v0.2.12, got v9.9.9", result.stderr)
    ...
    def test_release_evidence_bundle_rejects_bundle_version_mismatch(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertNotEqual(result.returncode, 0)
            self.assertIn("bundle m80_version mismatch: expected v0.2.12, got v9.9.9", result.stderr)
    ...
    def test_remote_release_asset_inventory_writes_remote_byte_digests(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(inventory["release_tag"], "v0.2.12")
            ...
            for asset in inventory["assets"]:
                ...
                self.assertTrue(asset["download_url"].endswith(f"/releases/download/v0.2.12/{asset['name']}"))
    ...
    def test_remote_release_rerun_preflight_rejects_partial_upload(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn(f"rerun preflight recovery_command=gh release delete v0.2.12 --yes", result.stderr)
            ...
            payload["assets"].append(
                {
                    ...
                    "name": "unexpected.txt",
                    "size": 1,
                    "browser_download_url": "https://github.com/moradology/m80/releases/download/v0.2.12/unexpected.txt",
    ...
    def test_rejects_binary_source_commit_mismatch(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            inputs = fixture_inputs(root, release_tag="v0.2.12")
            write_fake_m80(inputs["m80"], "v0.2.12", source_commit="1" * 40)
    ...
    def test_rejects_binary_target_mismatch(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            inputs = fixture_inputs(root, release_tag="v0.2.12")
            write_fake_m80(inputs["m80"], "v0.2.12", target="linux-aarch64")
    ...
    def test_rejects_binary_target_triple_mismatch(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            inputs = fixture_inputs(root, release_tag="v0.2.12")
            write_fake_m80(inputs["m80"], "v0.2.12", target_triple="aarch64-unknown-linux-gnu")
    ...
    def test_rejects_prerelease_release_tag(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            inputs = fixture_inputs(root, release_tag="v0.2.12")
            result = run_package(inputs, root / "out", release_tag="v0.2.12-rc.1", check=False)
    ...
    def test_rejects_guest_protocol_mismatch(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            inputs = fixture_inputs(root, release_tag="v0.2.12", guest_protocol=2)
            result = run_package(inputs, root / "out", check=False)
    ...
    def test_human_release_dist_verifier_rejects_wrong_tag(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertNotEqual(result.returncode, 0)
            self.assertIn("release tag mismatch: expected v0.2.12, got v9.9.9", result.stderr)
    ...
    def test_release_attestation_metadata_writer_accepts_verified_bundle(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(payload["release_tag"], "v0.2.12")
    ...
    def test_release_integrity_material_rejects_unknown_signer(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            rewrite_attestation_metadata(
                root / "out",
                {
                    "signer_identity": "repo:moradology/other:ref:refs/tags/v0.2.12",
                },
            )
...
def write_fake_guestd(path: Path, protocol_version: int) -> None:
    write_executable(path, f"#!/bin/sh\nprintf 'm80-guestd 0.2.12 (proto v{protocol_version})\\n'\n")
...
def add_alternate_image_kind_fixture(out_dir: Path, *, image_kind: str = "debug") -> None:
    ...
    asset.update(
        {
            "name": bundle_name,
            "url": f"https://github.com/moradology/m80/releases/download/v0.2.12/{bundle_name}",
        ...
        "predicate": {
            "buildDefinition": {
                ...
                "resolvedDependencies": [
                    {
                        "uri": "git+https://github.com/moradology/m80@refs/tags/v0.2.12",
            ...
            "runDetails": {
                "builder": {
                    "id": "https://github.com/moradology/m80/.github/workflows/release-artifacts.yml@refs/tags/v0.2.12"
                },
                "metadata": {
...
if bundle.get("release_tag") != "v0.2.12":
...
def assert_no_bundle_download(test: unittest.TestCase, urls: list[str], install_args: Path) -> None:
    bundle_url = f"https://github.com/moradology/m80/releases/download/v0.2.12/{BUNDLE_NAME}"
...
def assert_selector_index_drift_diagnostic(
    ...
    test.assertIn("release_tag=v0.2.12", stderr)
        ...
        json.dumps(
            {
                ...
                "decision": decision,
                "repository": "moradology/m80",
                "github_ref": "refs/tags/v0.2.12",
                "release_tag": "v0.2.12",
                "commit_sha": INTEGRITY_COMMIT_SHA,
                "workflow_ref": "moradology/m80/.github/workflows/release-artifacts.yml@refs/tags/v0.2.12",
                ...
                "probes": [
                    {
                        ...
                        "command": "gh release view v0.2.12 --repo moradology/m80 --json tagName,url,isDraft,isPrerelease",
                        "exit_status": 0,
                        "stdout": "{\"tagName\":\"v0.2.12\",\"isDraft\":false,\"isPrerelease\":false}",
                        "stderr": "",
                    }
                ],
        ...
        json.dumps(
            {
                ...
                "release": {
                    "requested": "v0.2.12",
                    "resolved_tag": "v0.2.12",
                    "install_url": "https://github.com/moradology/m80/releases/download/v0.2.12/install.sh",
                },
                "command": {
        ...
        assets.append(
            {
                ...
                "browser_download_url": f"https://github.com/moradology/m80/releases/download/v0.2.12/{name}",
                "created_at": "2026-05-21T00:00:00Z",
                "updated_at": "2026-05-21T00:00:01Z",
            }
        )
...
def run_release_publish_receipt(
    ...
    *args: str,
    ...
    github_ref: str = "refs/tags/v0.2.12",
) -> subprocess.CompletedProcess[str]:
    cmd = [
        "python3",
        str(PUBLISH_RECEIPT),
