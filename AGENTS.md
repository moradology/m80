...
- **No typed error collapse at public boundaries.** If a boundary needs to carry an m80 error class, it carries a finite typed variant plus detail, not `Other(String)` / `Generic` with prose. CLI exit-code mapping must stay exhaustive over `FcError`.
